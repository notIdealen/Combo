...
MstResult Build(const std::vector<Point>& terminals) const
{
	const size_t tNumber = terminals.size();
	MstResult result{};
	if (tNumber <= 1) return result;

	DisjointSetUnion disjointSet(tNumber);
	while (true)
	{
		const std::vector<std::optional<UndirectedEdge>> phaseBest =
			CollectPhaseMinimumEdges(terminals, tNumber, disjointSet);
		const int mergesThisPhase = ApplyPhaseMerges(disjointSet, result, tNumber, phaseBest);
