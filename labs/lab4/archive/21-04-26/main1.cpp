...
{
    SteinerTree sg{0};// привожу к лучшей версии
    int tNumbers = t.size();
        ...
        for (size_t b = a + 1; b < tNumbers; ++b)
        {
            ...
            for (Point X : pretendents)
            {
                for (size_t c = 0; c < tNumbers; ++c)
                {
                    ...
                    double edgeLen = Dist(sp.pos, A) + Dist(sp.pos, B);
                    // Полная длина = поддерево + рёбра к A и B
                    double totalLen = temp.length + edgeLen;
                    
                    if (totalLen < sg.length) {
                        sg.length = totalLen;
                        sg.sPoints = temp.sPoints;
                        sg.sPoints.push_back(sp);
                    }
                    // Point temp{sp.x, sp.y};
                    ...
                    // if (minLength == -1) minLength = sg.length;
                    // if (sg.length < minLength){}
                    //может ли быть прикрепление не к той точке штейнера?
                    // sg.sPoints.push_back(sp);
                }
            }
        }
