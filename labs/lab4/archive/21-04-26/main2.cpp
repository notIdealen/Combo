...
void ShowCoordsAndLength(const Point& A, const Point& B, const Point& C, Point& S, double len)
{
    cout << "TEST: ";
    cout << A.x << ' ' << A.y << " / ";
    cout << B.x << ' ' << B.y << " / ";
...
SteinerTree FindSteinerOnCircleAmongThreePoints(const Point& A, const Point& B, const Point& C) {
    ...
    for (const auto id : {P1.id, P2.id, P3.id})
...
SteinerTree FindSteinerAmongThreePoints(Point& A, Point& B, Point& C)
{
    ...
    { 
        ...
        sg.sPoints.push_back(sp);
        ShowCoordsAndLength(A, B, C, sp.pos, sg.length);////////////////////////////////////////////////////////////
        return sg;
    }
...
SteinerPoint FindSteinerPoint(Point& A, Point& B, Point& S, Point& X) {
    SteinerPoint sp;
    vector<std::string> ids;
    for (const auto id : {A.id, B.id, S.id})
        if (id != "") ids.push_back(id);
        ...
        for (size_t b = a + 1; b < tNumbers; ++b)
        {
            ...
            for (Point X : pretendents)
            {
                for (size_t c = 0; c < tNumbers; ++c)
                {
                    if (c == a || c == b) continue;
                    Point C = t[c];
                    if (!IsValidExternalVertex(A, B, C, X)) break;
                    // if (!IsPointOutsideCircle(A, B, X, C)) break;
                    ...
                    //есть точки A B C X temp.F
                    Point S = temp.sPoints[temp.sPoints.size() - 1].pos;
                    if (//temp.length < minLength && 
                        // IsValidExternalVertex(A, B, S, X) && 
                        // IsPointOutsideCircle(A, B, X, S) &&
                        12.0 < temp.length)
                    {
                        minLength = temp.length;
                        sg = temp;
                        SteinerPoint sp = FindSteinerPoint(A, B, S, X);
                        sp.AddEdgesIds({temp.sPoints[temp.sPoints.size() - 1].id}); 
                        sg.sPoints.push_back(sp);
                    }
                }
            }
